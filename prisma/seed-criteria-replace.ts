...
// ───────────────────────────────────────
// JSON fixer for table_schema
// ───────────────────────────────────────
function parseTableSchema(raw: string | null, lineNum: number): any {
  ...
  try {
    ...
    const fixed = raw
      .replace(/'/g, '"')
      .replace(/""/g, '"'); // fix double-double from single→double
    try {
...
function loadCsv(filename: string): CsvData {
...
async function main() {
  ...
  for (const [table, model] of Object.entries(MODEL_MAP)) {
    countsBefore[table] = await (prisma as any)[model].count();
    console.log(`  ${table}: ${countsBefore[table]}`);
  }

  // ── Step 4: Execute replacement in transaction ──
  console.log('\n🔄 Starting transaction...');

  await prisma.$transaction(async (tx) => {
    // 4a. Disconnect campaigns from templates
    console.log('  Disconnecting campaigns from templates...');
    await tx.$executeRawUnsafe(`UPDATE campaigns SET template_id = NULL`);

    // 4b. Delete inspection-related data
    ...
    // 4c. Delete criteria tables (reverse dependency order)
    ...
    // ── 4d. Insert new data (dependency order) ──
    ...
    for (let i = 0; i < csvDetails.rows.length; i++) {
      ...
      // Line number in CSV = i + 2 (1-indexed + header)
      const csvLine = i + 2;
      const tableSchema = parseTableSchema(r.table_schema || null, csvLine);
      if (tableSchema && r.table_schema && r.table_schema.includes("'")) fixedLines++;
  ...
  for (const [table, model] of Object.entries(MODEL_MAP)) {
    countsAfter[table] = await (prisma as any)[model].count();
